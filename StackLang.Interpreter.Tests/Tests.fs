...
let interpretMultiple (input: string list) =
    ...
    input
    |> List.fold (fun previousResult next ->
...
let assertError (expectedMessage: string) (result: Result<'a, string>) =
    match result with
    | Ok result ->
        Assert.True(false, $"Expected error but got {result}")
    | Error msg ->
...
let ``Addition requires two elements`` () =
    interpret "1 +"
    |> assertError "Stack underflow"

[<Fact>]
let ``Can add integers`` () =
    interpret "6 2 +"
    |> assertStackMatches [ Integer 8 ]

[<Fact>]
let ``Can add floats`` () =
    interpret "6.5 3.1 +"
    |> assertStackMatches [ Float 9.6 ]

[<Fact>]
let ``Can add strings`` () =
    interpret "\"hello\" \"world\" +"
    |> assertStackMatches [ String "helloworld" ]
...
let ``Can multiply integers`` () =
    interpret "3 2 *"
    |> assertStackMatches [ Integer 6 ]

[<Fact>]
let ``Can multiply floats`` () =
    interpret "3.3 4.2 *"
    |> assertStackMatches [ Float 13.86 ]
...
let ``Can divide integers`` () =
    interpret "8 4 /"
    |> assertStackMatches [ Integer 2 ]

[<Fact>]
let ``Can divide floats`` () =
    interpret "12.5 2.5 /"
    |> assertStackMatches [ Float 5.0 ]
...
let ``Can duplicate stack elements`` () =
    interpret "3 dup"
    |> assertStackMatches [ Integer 3; Integer 3 ]
...
let ``Can swap stack elements`` () =
    interpret "3 2 swap"
    |> assertStackMatches [ Integer 3; Integer 2 ]
